...
<html>
    ...
    <body>
        ...
        <form action="ManterMatriculaAcademiaController?acao=confirmarOperacao&operacao=${operacao}&idMatriculaAcademia=${matriculaAcademia.idMatriculaAcademia}" method="post" name="formManterMatriculaAcademia" onsubmit="return validarFormulario(this)">
                    ...
                    <td>
                        <select name="optAluno" <c:if test="${operacao == 'Excluir'}"> disabled </c:if>>
                            ...
                            <c:forEach items="${alunos}" var="aluno">
                                <option value="${aluno.idAluno}" <c:if test="${matriculaAcademia.aluno.idAluno == aluno.idAluno}"> selected </c:if>>${aluno.usuario.nome}</option>
                            </c:forEach>
                        </select>
                    </td>
                </tr>
